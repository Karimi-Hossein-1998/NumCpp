...
int main()
{
	...
	auto LU = LUDecomposeCompact(A);
	LUDecomposeInPlace(A);
	...
	std::println("LU=LUDecomposeSingle(A)"); LU.printm(8,4);
	std::println("A (after LUDecomposeSingleInPlace)"); A.printm(8,4);

	std::println("Matrix.UTMatrix"); (MatMul(LN,U)).printm();
	std::println("LTMatrix.Matrix"); (MatMul(L,UN)).printm();
