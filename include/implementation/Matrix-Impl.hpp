...
template <FPNumber T>
inline void LUSolveInPlaceNormal(const Matrix<T>& L, const Matrix<T>& U, Matrix<T>& RHSMatrix)
{
...
template <FPNumber T>
inline Matrix<T> LUSolveNormal(const Matrix<T>& L, const Matrix<T>& U, const Matrix<T>& RHSMatrix) {Matrix<T> b(RHSMatrix); LUSolveInPlaceNormal(L,U,b); return b;}

///////      ******* USING LT- and UT- Matrix CLASSES *******       ///////
